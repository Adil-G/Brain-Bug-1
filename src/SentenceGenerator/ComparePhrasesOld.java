 ...
 */
public class ComparePhrasesOld {
...
//
                    if ( graph0.getVertex(label) != null && graph1.getVertex(label) != null) {
                        ...
                        score += GraphNew_July8.print(graph0.getVertex(label), graph1.getVertex(label), label, Graph.START_GRAPH_SEARCH,0,0,new HashSet<>(),false
                                ,0);
                        ...
                        if(ComparePhrases.synMap.containsKey(label))
                            for(String synonym : ComparePhrases.synMap.get(label))
                                if (
                                    ...
                                    System.out.println("309i30---->> "+graph0.getVertex(label).getNeighbors());
                                    System.out.println("309i31---->> "+Arrays.asList(arrayA));
                                    score += GraphNew_July8.print(graph0.getVertex(label), graph1.getVertex(synonym), label, Graph.START_GRAPH_SEARCH,0,0,new HashSet<>(),false
                                            ,0);
                ...
                if(!hasAUniqueKeyWord
                    //||phrase1.toLowerCase().replaceAll("[^\\w^\\s]+", "").replaceAll("\\[.*?\\]","").trim().contains("this ")
                    //||phrase1.toLowerCase().replaceAll("[^\\w^\\s]+", "").replaceAll("\\[.*?\\]","").trim().contains(" this")
                        )
                    score = 0;
