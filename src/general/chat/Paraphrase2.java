...
public class Paraphrase2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(String input : inputO.replaceAll("\\n","<adsabas>").split("[\\.]")) {
            ...
            String newText = new Paraphrase2().operation1(input, new String());
    ...
    public String paraphrase(String inputO )throws IOException {
        map =new SynonymMap(new FileInputStream("openNLP/wn_s.pl"));
        String fullOutput = "";
        for(String input : inputO.replaceAll("\\n","<adsabas>").split("[\\.]")) {
            System.out.println("sadfadsf");
    ...
    public  String rephraseString(String input, String textDataBase) throws IOException {
        if(map!=null) {
            //nlpToWordNet.put("NN", )
            //morphy_tag = {'NN':wordnet.NOUN,'JJ':wordnet.ADJ,'VB':wordnet.VERB,'RB':wordnet.ADV}[penn_tag[:2]]
            String fullText = "";
    ...
    public  String POSTag2(String input) throws IOException {
    ...
    public  Map<String,ArrayList<String>> POSTag2X(String[] words,Map<String,ArrayList<String>> totalPhrase,
    ...
    public  String operation1(String text, String textDataBase) throws IOException {
        ...
        for (String x : words) {
            ...
            if (!ms.matches()) {
               ...
               // int frequency = StringUtils.countMatches(textDataBase.toLowerCase(),
                ...
                if(!tag.contains("DT")&&!tag.contains("IN")&&(x.length() >2) && ((true||chance < 0.75))) {
                   // System.out.println("324234 frequency of " + x.replaceAll("_[A-Z\\$]+", "").toLowerCase() + "- " + frequency);
                    ...
                    if (synonyms.size() == 0) {
                        synonymCounter--;
                        synonymCounter = Math.max(synonymCounter,0);
                        //synonyms = listSynonymsJJ2(x, tag);
                    }
                    else
                    {
                        synonymCounter ++;
                        synonymCounter = Math.min(synonymCounter,2);
                    }
                    x = x.replaceAll("_[A-Z\\$]+", "");
                    ...
                    if(synonymCounter-1<=0)
                        totalPhrase.put(x, new ArrayList<>(synonyms));
                    else
                        totalPhrase.put(x, new ArrayList<>());
    ...
    public  ArrayList<String> listSynonymsJJ4(String lastWord) throws IOException {
...
*/
    ...
    public  ArrayList<String> listSynonymsJJ2(String lastWord,String type) throws IOException {
    ...
    public  String filterWithTag2(String[] words,Map<String,ArrayList<String>> totalPhrase,
                                              Map<String,String> wordToTag) throws IOException {
        ...
        for (String x : words) {
            ...
            if(newWords.get(x)!=null&&newWords.get(x).size()>0)
            {
